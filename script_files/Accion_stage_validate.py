...
logger = get_logger("Accion_stage_validate")
...
def row_count_valid(df_rc,tb_rc):
    logger.info("validating the record count")
    if(df_rc == tb_rc):
        #print('data has been dumped correctly')
        logger.info('stage validation - success')
    else:
        #print('data is not inserted properly')
        logger.error('stage validation - failed')
        exit(0)

def validate_stage():
    logger.info("validating the stage table")
    rc = get_dataframe_count()
    count_sales = get_row_count('sales.accion_sales_stage')
