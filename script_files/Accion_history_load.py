...
logger = get_logger("Accion_history_load")
...
def get_data_table(table_name):
    ...
    try:
        ...
        df.to_sql(lc,db_session,schema='sales',if_exists='append',index=False,chunksize=10000)
...
def load_history():
    logger.info("Loading the dataframe to the history table")
    data = get_data_table('sales.accion_sales_stage')
    logger.info("Data loaded successfully to the history table")
